# Reglas de Antigravity en Termux / Nix-on-Droid
...
- **Al finalizar cada tarea o petición del usuario**:
  ...
  3. **Estilo de locución**: Sé natural y directo. No es necesario decir siempre "Terminé la tarea"; ve directamente a explicar la acción realizada o la respuesta (por ejemplo: *"He modificado la regla en tu archivo..."*, *"Acabo de analizar los parámetros..."*, *"Configuré el acceso a..."*).

## Capacidades y Habilidades del Sistema Android
1. **Capturas de Pantalla (Screenshots)**: Toma capturas en tiempo real con `$HOME/bin/rish -c "/system/bin/screencap -p <ruta.png>"` e inspecciónalas con `view_file` para diagnóstico visual del teléfono.
2. **Comandos ADB Nativos vía Shizuku**: Ejecuta comandos como `pm`, `am`, `settings`, `dumpsys` mediante `$HOME/bin/rish -c "<comando>"` sin requerir servidores ADB ni cables.
